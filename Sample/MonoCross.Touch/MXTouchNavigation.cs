...
namespace MonoCross.Touch
{
	...
	public class MXTouchNavigation
	{
		...
		public void Init(UIViewController viewController)
		{
			...
			_masterNavigationController.NavigationBar.TintColor = UIColor.FromRGB(0, 63, 107);
		...
		public UITabBarController TabBar { get { return _tabBarController; }}
		internal void SetTabBarControler(UITabBarController tabBar) 
		{ 
			_tabBarController = tabBar;
			this.View = _tabBarController.View;
		}
		...
		public void PushToModel(UIViewController vc)
		{
			...
			else if (_masterNavigationController != null)
			{
				_masterNavigationController.PresentModalViewController(vc, true);
			}
			else if (_tabBarController != null) 
			{
				if (_tabBarController.ModalViewController != vc)
					_tabBarController.PresentModalViewController(vc, true); 
				//else
					//throw new ApplicationException("View is already in ModalViewController: " + vc.GetType().ToString());
			}
			else { throw new ApplicationException("Unexpected MonoCross framework Exception pushing to Modal"); }
		}
		
		public void PushViewInTabBar(UIViewController vc, bool animated)
		{
