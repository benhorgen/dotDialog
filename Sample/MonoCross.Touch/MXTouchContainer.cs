...
namespace MonoCross.Touch
{
	...
	public class MXTouchContainer: MXContainer
	{
		...
		public void SetTabBarControllerAsRoot(UITabBarController tabBar) 
		{ 
			_touchNavigation.SetTabBarControler(tabBar); 
		}		

		public UITabBarController GetTabBarController() { return _touchNavigation.TabBar; }
		...
		protected override void OnControllerLoadFailed (IMXController controller, Exception ex)
		{
			...
			throw ex;
#else
			// Fixes an issue when navigation from an event not on the main thread.
			new NSObject().InvokeOnMainThread( () => {
				new UIAlertView("Load Failed", ex.Message, null, "OK", null).Show();
			});
#endif
		}
